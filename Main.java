...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < data.length; i++) {
            if (isNumeric( data[i]) && (Integer.valueOf(data[i])>10 || Integer.valueOf(data[i])<1))
            {
                throw new Exception("Введено число больше 10 или меньше 1");
            }

            data[i] = data[i].replace("\"", "");
            if(!(isNumeric( data[i])) && data[i].length()>10)
            {
                throw new Exception("Введена строка больше 10 символов");
            }
        }
